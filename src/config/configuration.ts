...
export const envValidationSchema = Joi.object({
  ...
  ADMIN_EMAIL: Joi.string().email().default('pranav@admin.com'),
  ADMIN_PASSWORD: Joi.string().min(8).default('Pranav!123'),
  ADMIN_NAME: Joi.string().default('Pranav Admin'),
...
export default () => ({
  ...
  admin: {
    email: trimEnv(process.env.ADMIN_EMAIL) ?? 'pranav@admin.com',
    password: trimEnv(process.env.ADMIN_PASSWORD) ?? 'Pranav!123',
    name: trimEnv(process.env.ADMIN_NAME) ?? 'Pranav Admin',
  },
  firebase: {
